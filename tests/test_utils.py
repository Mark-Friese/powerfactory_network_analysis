...
from src.models.network_element import ElementType, Region
...
class TestUtils(unittest.TestCase):
    ...
    def test_validator_file_path_validation(self):
        ...
        missing_file = self.temp_path / "missing.txt"
        is_valid, error = self.validator.validate_file_path(missing_file, must_exist=True)
        self.assertFalse(is_valid)
    ...
    def test_validator_numeric_range_validation(self):
        ...
        is_valid, error = self.validator.validate_numeric_range(50, 0, 100, "test_value")
        self.assertTrue(is_valid)
        self.assertEqual(error, "")
        
        # Value below minimum
        is_valid, error = self.validator.validate_numeric_range(-5, 0, 100, "test_value")
        self.assertFalse(is_valid)
        self.assertIn("must be >= 0", error)
        
        # Value above maximum
        is_valid, error = self.validator.validate_numeric_range(150, 0, 100, "test_value")
        self.assertFalse(is_valid)
        self.assertIn("must be <= 100", error)
        
        # Non-numeric value
        is_valid, error = self.validator.validate_numeric_range("not_a_number", 0, 100, "test_value")
        self.assertFalse(is_valid)
    ...
    def test_validator_string_format_validation(self):
        ...
        # Valid string
        is_valid, error = self.validator.validate_string_format("valid_string", value_name="test")
        self.assertTrue(is_valid)
        self.assertEqual(error, "")
        
        # String length validation
        is_valid, error = self.validator.validate_string_format("short", min_length=10, value_name="test")
        self.assertFalse(is_valid)
        self.assertIn("at least 10 characters", error)
        
        is_valid, error = self.validator.validate_string_format("very_long_string", max_length=5, value_name="test")
        self.assertFalse(is_valid)
    ...
    def test_validator_enum_validation(self):
        ...
        # Valid enum value
        is_valid, error = self.validator.validate_enum_value(ElementType.LINE, ElementType, "element_type")
        self.assertTrue(is_valid)
        self.assertEqual(error, "")
        
        # Valid string representation
        is_valid, error = self.validator.validate_enum_value("LINE", ElementType, "element_type")
        self.assertTrue(is_valid)
        
        # Invalid value
        is_valid, error = self.validator.validate_enum_value("INVALID_TYPE", ElementType, "element_type")
        self.assertFalse(is_valid)
    ...
    def test_validator_voltage_level_validation(self):
        ...
        for level in valid_levels:
            is_valid, error = self.validator.validate_voltage_level(level, "voltage_level")
    ...
    def test_validator_datetime_string_validation(self):
        ...
        is_valid, error = self.validator.validate_datetime_string(valid_datetime, "%Y-%m-%d %H:%M:%S", "timestamp")
        self.assertTrue(is_valid)
        
        # Invalid format
        invalid_datetime = "25/12/2023 3:30 PM"
        is_valid, error = self.validator.validate_datetime_string(invalid_datetime, "%Y-%m-%d %H:%M:%S", "timestamp")
        self.assertFalse(is_valid)
